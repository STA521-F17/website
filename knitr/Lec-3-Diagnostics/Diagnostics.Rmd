...
$$ h_{ii} = 1/n + (\x_i - \xbar)^T \left((\X- \one \xbar^T) ^T(\X - \one \xbar^T)\right)^{-1}(\x_i - \xbar)$$
...
$$ t_i = \frac{e_{(i)}}{\sqrt{\shat_{(i)}/(1 - h_{ii})}}  =  \frac{y_i  - \x_i^T \bhat_{(i)}} {\sqrt{\shat_{(i)}/(1 - h_{ii})}}  =  r_i \left(\frac{ n - p - 1}{n - p - r_i^2}\right)^{1/2}$$  
